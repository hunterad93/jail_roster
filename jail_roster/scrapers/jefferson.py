...
BASE_URL = "https://jefferson-so-mt.zuercherportal.com/api/portal/inmates"
JAIL_NAME = "Jefferson County"
PAGE_SIZE = 50
...
def _parse_hold_reasons(html: str) -> tuple[str, str]:
    ...
    for block in html.split("<br />"):
        ...
        if not block:
            continue
        charge_match = re.search(r"Charge:\s*[\d\-()a-zA-Z\[\]]+ - (.+?)(?:;|$)", block)
        if charge_match:
            desc = charge_match.group(1).strip().rstrip(";")
            if desc:
                charges.append(desc)
        warrant_match = re.search(r"Local Warrant:.*?\(.*? - (.+?)\)", block)
        if warrant_match:
            charges.append(warrant_match.group(1).strip())
        hold_match = re.search(r"Additional Hold for (.+?);", block)
        if hold_match:
            charges.append(f"Hold: {hold_match.group(1).strip()}")
